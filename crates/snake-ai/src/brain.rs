...
impl WorkerPool {
    ...
    fn run(&mut self, num_threads: usize, task: &SearchTask) -> (crate::search::SearchResult, crate::PerfStats) {
...
fn execute_search_task(thread_id: usize, mut task: SearchTask, history: &mut [Vec<i32>; 2], buffers: &mut SearchBuffers) -> WorkerResult {
    ...
    while let Ok(command) = rx.recv() {
        match command {
            WorkerCommand::Search { task, result_tx } => {
                let result = execute_search_task(thread_id, task, &mut history, &mut buffers);
...
pub fn decide_move_debug(me: AgentState, enemy: AgentState, foods: Vec<Point>, cols: i32, rows: i32, cfg: &AiConfig) -> Decision {
    ...
    let (primary_res, aggregated_stats) = if num_threads <= 1 {
        ...
        pool.run(num_threads, &base_task)
    };
