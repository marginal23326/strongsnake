...
pub fn negamax(
    ...
    if depth == 0 {
        ...
        let score = if side == 0 {
            evaluate(grid, me, enemy, food, dist_map, ctx.cfg, ctx.buffers)
        } else {
            -evaluate(grid, enemy, me, food, dist_map, ctx.cfg, ctx.buffers)
        };
    ...
    for (i, &mv) in moves.iter().enumerate() {
        ...
        if ate_food && let Some(idx) = food.iter().position(|f| f.x == mv.x && f.y == mv.y) {
